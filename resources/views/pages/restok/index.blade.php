    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Data restok</h4>
                <button data-toggle="modal" data-target="#basicModal" type="button" class="btn btn-primary">Restok Produk</button>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table id="example" class="display min-w850">
                        ...
                        <tbody>
                            @foreach ($restok as $index => $restok)
                                <tr>
                                    <td>{{ $index +1 }}</td>
                                    <td>{{ $restok->produk->nama }}</td>
                                    <td>{{ $restok->supplier->nama }}</td>
                                    <td>{{ $restok->qty }}</td>
                                    <td>{{ $restok->tanggal }}</td>
                                    <td>
                                        <div class="d-flex">
                                            <form method="POST" action="{{ route('restok.destroy', $restok->id) }}">
                                                @csrf
                                                @method('DELETE')
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">Restok Produk</h5>
                            ...
                            <div class="modal-body">
                                <form method="POST" action="{{ route('restok.store') }}" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input name="tanggal" type="text" class="form-control" placeholder="Masukan tanggal restok" id="mdate">
                                    </div>
